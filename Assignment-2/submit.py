...
def my_fit( words, verbose = False ):
	dt = Tree( min_leaf_size = 1, max_depth = 12 )		
	mywords = [[i, words[i]] for i in range(len(words))]
	dt.fit( words, mywords,  verbose )
	return dt
...
class Tree:
	...
	def fit( self,  words, mywordss, verbose = False ):
		self.root = Node( depth = 0, parent = None )
		if verbose:
			print( "root" )
			print( "└───", end = '' )
		self.root.fit(  mywords = mywordss, min_leaf_size = self.min_leaf_size, max_depth = self.max_depth, verbose = verbose )
...
class Node:
	...
	def process_leaf( self, mywords, history ):
		return mywords[0][0]
	...
	def get_entropy(self,counts):
		...
		if num_elements <= 1:
			...
			return 0
		coo = np.array(counts)
		proportions = coo / num_elements
		return np.sum( proportions * np.log2( coo ) )

	def process_node( self, mywords,  history, verbose ):
		...
		if len( history ) == 0:
			...
			index = 0
			best_score = -1
			arr = np.zeros((len(mywords[0][1]), 26))
			for word in mywords:
				inddd = 0
				for index in word[1]:
					arr[inddd][ord(index)-ord('a')] +=1
					inddd +=1
			for idxx in mywords:
				if idxx[0] not in history:
					score = 0
					indexx = 0
					for char in idxx[1]:
						score += arr[indexx][ord(char)-ord('a')]
						indexx += 1
					if score > best_score:
						best_score = score
						query_idx = idxx[0]
	...
	def fit( self,  mywords, min_leaf_size, max_depth, fmt_str = "    ", verbose = False ):
		...
		if len( mywords ) <= min_leaf_size or self.depth >= max_depth:
			self.is_leaf = True
			self.query_idx = self.process_leaf( mywords, self.history )
			if verbose:
				print( '█' )
		else:
			self.is_leaf = False
			( self.query_idx, split_dict ) = self.process_node( mywords,    self.history, verbose )
